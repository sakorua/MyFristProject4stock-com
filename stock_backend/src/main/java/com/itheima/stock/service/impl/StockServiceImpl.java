...
//        }).collect(Collectors.toList());
     ...
     * @Date 1:59 PM 2022/1/17
     * @Param
     **/
    @Override
    public R<Map> stockDkLine4Week(String code) {
        Date open4Mon = DateTime.now().withDayOfWeek(3).withHourOfDay(9).withMinuteOfHour(30).withSecondOfMinute(0).toDate();
        Date close4Sun = DateTime.now().withDayOfWeek(7).withHourOfDay(14).withMinuteOfHour(58).withSecondOfMinute(0).toDate();

        Map map = stockRtInfoMapper.stockDkLine4Week(code);
        return R.ok(map);
    }
